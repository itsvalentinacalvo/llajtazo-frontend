...
import { Spacing, Typography, Shadows } from "@/src/core/constants/theme";

type IconFamily = "ionicons" | "fontawesome" | "materialcommunityicons";
...
const TAB_CONFIG: TabConfig[] = [
  { name: "InicioTab", label: "Inicio", icon: "home", iconFamily: "ionicons" },
  { name: "EventosTab", label: "Eventos", icon: "calendar", iconFamily: "ionicons" },
  { name: "NuevoTab", label: "Nuevo", icon: "add-circle", iconFamily: "ionicons" },
  { name: "MetricasTab", label: "Métricas", icon: "chart-line", iconFamily: "materialcommunityicons" },
...
function AnimatedTabItem({
  ...
  const newTabAnimatedStyle = useAnimatedStyle(() => {
    const translateX = interpolate(
      animationProgress.value,
      [0, 1],
      [translateDirection * 3, 0]
  ...
  const renderIcon = () => {
    switch (tabConfig.iconFamily) {
      ...
      case "materialcommunityicons":
        return <MaterialCommunityIcons name={tabConfig.icon as any} size={24} color={iconColor} />;
      default:
        return null;
    }
  };

  if (tabConfig.name === "NuevoTab") {
    return (
      <Pressable
        key={route.key}
      ...
      >
        ...
        >
          <Entypo name="squared-plus" size={24} color={theme.buttonText} />
        </Animated.View>
      </Pressable>
    );
  }
...
function CoreTabBarBusinessImpl({ state, descriptors, navigation }: BottomTabBarProps) {
  const insets = useSafeAreaInsets();
  const previousIndexRef = useRef(state.index);

  console.debug("[Core][CoreTabBarBusiness] render", { index: state.index, routes: state.routes.map(r => r.name) });
...
export const CoreTabBarBusiness = React.memo(CoreTabBarBusinessImpl);

export default CoreTabBarBusiness;
