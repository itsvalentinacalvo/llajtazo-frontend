...
type VerificationBusinessNavigation = NativeStackNavigationProp<
  BusinessAuthStackParamList,
  "VerificationBusiness"
>;

type VerificationBusinessRoute = RouteProp<
  BusinessAuthStackParamList,
  "VerificationBusiness"
>;

export default function VerificationBusinessScreen({
  onAuthSuccess,
}: {
  onAuthSuccess?: () => void;
}) {
  const navigation = useNavigation<VerificationBusinessNavigation>();
  const route = useRoute<VerificationBusinessRoute>();
  const insets = useSafeAreaInsets();
  const { verificationCode, completePendingVerification } = useBusiness();
  ...
  const handleContinue = () => {
    ...
    const completed = completePendingVerification();
    if (!completed) {
      setError("No encontramos una acción pendiente");
      triggerShake();
      return;
    }
  ...
  const handleResend = () => {
    if (timer === 0) {
      setTimer(20);
      setError(undefined);
      console.log("[Business][Verification] Resend code");
    }
  };
