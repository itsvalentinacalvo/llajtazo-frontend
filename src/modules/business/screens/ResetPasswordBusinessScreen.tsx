...
import { BusinessAuthStackParamList } from "@/src/modules/business/navigation/BusinessAuthNavigator";
 ...
 type ResetPasswordBusinessNavigation = NativeStackNavigationProp<
  BusinessAuthStackParamList,
  "ResetPasswordBusiness"
>;

export default function ResetPasswordBusinessScreen() {
  const navigation = useNavigation<ResetPasswordBusinessNavigation>();
  const insets = useSafeAreaInsets();
  const { startBusinessPasswordReset } = useBusiness();
  ...
  const handleSend = () => {
    ...
    if (email.toLowerCase() !== BUSINESS_TEST_CREDENTIALS.email.toLowerCase()) {
    ...
    setError(undefined);
    startBusinessPasswordReset({ email });
    navigation.navigate("VerificationBusiness", { email, flow: "reset" });
  };
